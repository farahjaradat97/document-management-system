...
import SingleFilePreview from "@/Components/Projects/SingleFilePreview.vue";
...
const showFilePreview=ref(null)
...
<template>
    ...
    <AuthenticatedLayout :title="projectName" :bread-crumbs="crumbs">
        <template #action>
            <div class="flex justify-end gap-2 w-full">
                ...
                <Link :href="folders.data.parent_id
        ...
        <template #content>
            <Toaster :message="toasterMessage" v-if="showToaster" type="success">
            </Toaster>
            <div class="flex w-full gap-12 ">
                <div class="w-full h-screen ">
                    <table class="" v-if="files?.data?.length || search">
                        <thead>
                        ...
                        <tbody>
                            <tr @click="openFolder(file)" v-for="file of files?.data" :key="file.id">
                                <td>
                                    <div class="flex items-center gap-2 justify-start">
                                        <i v-if="file.is_folder" class="mdi mdi-folder text-primary text-4xl">
                                        </i>
                                        <i v-else class="mdi mdi-file-pdf-box text-error text-4xl">
                                        </i>
                                        <p class="text-lg capitalize font-semiBold text-dark-gray-900">
                                            {{ file.name }}
                                        </p>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div v-if="file.is_folder"
                                        class="flex items-start gap-1 text-primary flex justify-start font-semibold text-sm"
                                        @click.stop="openCreateFolderModalFun(file)">
                                        <i class="mdi mdi-square-edit-outline cursor-pointer text-primary"></i>

                                    </div>
                                    <div v-else
                                        class="flex items-start gap-1 text-primary flex justify-start font-semibold text-sm"
                                        @click.stop="route('projects.showEditForm', {
                                            id: projectId,
                                            folders: encodeURIComponent(
                                                folders.data.path
                                            ),
                                        ...
                                        <Link
                                        :href="route('projects.showEditForm', {
                                            id: projectId,
                                            folders: encodeURIComponent(
                                                folders.data.path
                                            ),
                        ...
                        <tr v-if="!files?.data.length && search" rowspan="12" class="justify-end bg-white">
                            <th colspan="12" class="font-medium">
                    ...
                    <div v-else>
                        <NoDataFound class="border border-light-gray-200"
                            message="No Folders or Files have been created yet." />
                    </div>
                </div>
                <div class="w-1/2 " v-if="showFilePreview">
                    <SingleFilePreview
                    @close="showFilePreview=false"
                    :file="selectedFile?.data?.file" />
                </div>
            </div>
    ...
    <Modal :title="folderToEdit ? 'Update Folder' : 'Create Folder'" max-width="md" :show="openCreateFolderModal"
        @close="openCreateFolderModal = false">
        <CreateOrUpdateFolder @showToaster="handelToasterMessage" :project-id="projectId"
            @close="openCreateFolderModal = false" :folder="folderToEdit" :parent-folder="folders.data" />
    </Modal>
</template>
